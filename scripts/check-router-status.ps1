param(
  ...
  [string]$LogPath = "C:\tmp\home-edge-router-status.log",
...
function Invoke-OptionalGit {
  ...
  try {
    # A source archive is intentionally not a Git worktree. Native Git stderr
    # must therefore be treated as probe output, not as a terminating error.
    $ErrorActionPreference = "SilentlyContinue"
    $Output = (& $GitCommand.Source @Arguments 2>$null | Out-String).Trim()
...
$GitProbe = Invoke-OptionalGit -Arguments @("-C", $Repo, "rev-parse", "--is-inside-work-tree")
if ($GitProbe.ExitCode -eq 0) {
  $GitStatus = Invoke-OptionalGit -Arguments @("-C", $Repo, "status", "--porcelain", "--untracked-files=no")
  if ($GitStatus.ExitCode -eq 0 -and -not $GitStatus.Output) {
    $GitHead = Invoke-OptionalGit -Arguments @("-C", $Repo, "rev-parse", "HEAD")
    if ($GitHead.ExitCode -eq 0 -and $GitHead.Output -match '^[0-9a-f]{40,64}$') {
      $ExpectedSourceKind = "git"
      $ExpectedSourceCommit = $GitHead.Output
    }
  }
}
else {
  ...
  if (Test-Path -LiteralPath $VersionPath -PathType Leaf) {
    ...
    if ($Candidate -match '^v[0-9]+\.[0-9]+\.[0-9]+$') { $ExpectedSourceKind = "release"; $ExpectedSourceVersion = $Candidate }
  }
}
...
try {
  ...
  Write-Host ""
  Write-Host "Router status check finished with exit code: $exitCode"
}
catch {
  $exitCode = 1
  if (-not $NoLog) {
  ...
  Write-Host ""
  Write-Host "Router status check failed:"
  Write-Host $_
}
finally {
  Write-Host ""
  if ($NoLog) {
    Write-Host "Status log: disabled"
  }
  else {
