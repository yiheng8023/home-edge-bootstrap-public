...
def normalized_archive(archive, source, prefix, epoch):
    ...
    with tarfile.open(archive,"r:gz") as tf:
        for m in tf.getmembers():
            ...
            expected_mode=0o755 if pathlib.PurePosixPath(rel).suffix == ".sh" else 0o644
            if m.mode != expected_mode: raise SystemExit("source archive file mode mismatch")
            if rel in seen: raise SystemExit("duplicate source archive path")
            if rel not in expected: raise SystemExit("source archive/source tree mismatch")
            archived=hashlib.sha256(tf.extractfile(m).read()).hexdigest()
            if archived != sha(expected[rel]): raise SystemExit("source archive/source tree mismatch")
            seen.add(rel)
...
mihomo_source=tree/paths["mihomo-linux-arm64"]["source"]
vendor=mihomo_source/"vendor"
...
spdx_ids={"mihomo-linux-arm64":"SPDXRef-Package-Mihomo","shellcrash":"SPDXRef-Package-ShellCrash"}
for cid,c in components.items():
...
try {
  ...
  & $Python $Driver ([System.IO.Path]::GetFullPath($Tree)) ([System.IO.Path]::GetFullPath($Lock)) ([System.IO.Path]::GetFullPath($Sbom)) ([System.IO.Path]::GetFullPath($Notice))
